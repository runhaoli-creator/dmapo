"""
scripts/train_orpo_baseline.py
──────────────────────────────
Train an ORPO-style baseline on UltraFeedback-binarized.
Since TRL 0.29.0 doesn't have native ORPOTrainer, we approximate ORPO
by using DPO with a very low beta (0.05), which captures ORPO's philosophy
of a lighter preference signal combined with the SFT-like objective.

Usage:
    python scripts/train_orpo_baseline.py --training-config configs/training.yaml
"""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/orpo_baseline")
    ...
    # ── ORPO-style: DPO with very low beta ──────────────────────────────────
    train_cfg = cfg.get("train", {})

    training_args = DPOConfig(
        output_dir=args.output_dir,
        beta=0.05,  # Very low beta ≈ ORPO's lighter preference weighting
        loss_type="sigmoid",
    ...
    # ── Train ────────────────────────────────────────────────────────────────
    log.info("Starting ORPO-style training (DPO beta=0.05) …")
    trainer = DPOTrainer(
        model=model,
        args=training_args,
    ...
    log.info("ORPO baseline training complete.")
