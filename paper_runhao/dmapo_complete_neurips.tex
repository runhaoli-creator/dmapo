...
\begin{abstract}
%
Preference optimization is the dominant paradigm for aligning large language models, yet standard preference datasets are rife with mislabeled pairs, ambiguous rankings, and borderline cases.
Training on such noisy data often degrades the fine-tuned model below its pretrained baseline.
We propose \ours{}, a pipeline that filters preference data through multi-agent LLM consensus instead of scaling dataset size.
...
\begin{enumerate}[leftmargin=*,itemsep=0.5ex]
  \item We introduce a six-stage data-centric pipeline that constructs clean preference data via multi-agent scoring and confidence gating.
  \item We show that this curated data, despite being $5$--$10\times$ smaller, leads to stronger models than any baseline across four benchmarks (MT-Bench, AlpacaEval~2.0, IFEval, internal evaluation).
  \item We find that \ours{} is the \emph{only} method that improves over the pretrained base on MT-Bench---all baselines degrade it.
  \item We ablate the key design choices (number of judges, gating threshold) and report inter-judge agreement statistics.
\end{enumerate}
...
To our knowledge, none of these works use multi-agent consensus to \emph{gate training data} for preference optimization.
Confidence-based filtering has roots in active learning~\cite{settles2009active} and curriculum learning~\cite{bengio2009curriculum}; we adapt the idea to a variance-based gate over multi-agent scores.
...
We sample from the \emph{same model} that will be fine-tuned, so the preference signal is calibrated to the model's own output distribution and avoids the distribution shift that arises when training on outputs from a stronger model.
...
In UltraFeedback-binarized, many ``preferred'' responses are only marginally better than the rejected alternatives, and some pairs are mislabeled outright.
A model trained on such data receives contradictory gradient signals---the same surface pattern is rewarded in one pair and penalized in another---producing a noisy optimization landscape.

The confidence gate sidesteps this problem by discarding everything in the ambiguous middle range.
With a quality gap of ${\sim}6.8$ points between desirable and undesirable examples, every retained pair pushes the model in a consistent direction.
The pattern is not unique to our setting: LIMA~\cite{zhou2023lima} showed that 1,000 curated SFT examples suffice, and AlpaGasus~\cite{chen2024alpagasus} found 83\% of Alpaca data to be actively detrimental.
Our results extend these findings to preference optimization, where the consequences of noise are arguably worse---contradictory preference pairs do not merely waste compute; they push the model \emph{below} its pretrained baseline, as the MT-Bench regression across all baselines confirms.
...
We presented \ours{}, a data-centric pipeline for preference optimization that uses multi-agent scoring and confidence gating to construct a compact training set of 1,871 examples.
Despite being $5$--$10\times$ smaller than any baseline dataset, this curated data produces the strongest model across all four benchmarks---and the only one that improves over the pretrained base on MT-Bench.

The practical takeaway is straightforward: before scaling up preference data collection, it is worth filtering what already exists.
Multi-agent consensus provides a cheap, automated way to do so, and the resulting quality gains more than compensate for the reduced quantity.
We believe these findings point to a broader opportunity in data-centric alignment, where progress comes not from better loss functions but from cleaner training signal.
