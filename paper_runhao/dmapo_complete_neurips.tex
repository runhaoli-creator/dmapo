...
Despite using 5.3$\times$ fewer training examples than any baseline, \ours{} achieves the highest internal preference win-rate (90.7\% vs.\ 87.6\% for SimPO), and remains competitive on general benchmarks: MT-Bench 7.30, AlpacaEval 2.0 95.0\%, IFEval 54.3\%.
...
\paragraph{Internal win-rate.}
\ours{} achieves the highest internal preference win-rate (90.7\%), outperforming SimPO (87.6\%) by 3.1 percentage points despite using 5.3$\times$ less training data.
This is the metric most directly aligned with our objective: preference quality on held-out examples from the same distribution as the gating pipeline.

\paragraph{General benchmarks.}
On general benchmarks, \ours{} remains competitive while using far less data.
ORPO leads on MT-Bench (7.42 vs.\ 7.30) and AlpacaEval (96.3\% vs.\ 95.0\%), and DPO leads on IFEval (54.9\% vs.\ 54.3\%); \ours{} is within the competitive range on all three.
...
\ours{} achieves the best Reasoning score (7.45) across all methods, including the pretrained base (7.15), which is the one category where quality gating most clearly helps---the process critic systematically filters logically flawed responses.
ORPO leads on Coding (6.40) and STEM (8.40), and KTO/ORPO tie for Humanities (8.85).
The pattern suggests that \ours{}'s gains are concentrated in categories requiring logical coherence, while stylistic categories (Writing, Roleplay) are less sensitive to quality gating.
...
\subsection{Why General Benchmarks Show Smaller Gains}

\ours{}'s strongest result is on the internal preference metric (+90.7\%), which directly measures alignment quality on held-out examples from the same gated distribution.
General benchmarks (MT-Bench, AlpacaEval, IFEval) measure broader capabilities that go beyond what quality filtering directly targets.
ORPO's MT-Bench lead (+0.12 over \ours{}) is likely due to its loss formulation combining SFT and preference signals simultaneously, which maintains general capability while optimizing preferences.
This suggests that \ours{}'s data filtering approach is complementary to algorithm design: combining quality-gated data with ORPO or SimPO's training objectives is a natural direction for future work.
...
\ours{} demonstrates that multi-agent consensus filtering can reduce a 54k-candidate pool to 1,871 high-quality examples that achieve a 90.7\% internal preference win-rate---outperforming all five baselines that use 5--10$\times$ more data---while remaining competitive on general benchmarks with 5.3$\times$ less training data.
